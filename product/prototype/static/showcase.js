...
let study, segment='All travellers', personId='D0001', ragSelection='context';
...
function renderRetrieval(){
 ...
 $('passages').innerHTML=r.chunks.length?r.chunks.map(c=>`<article class="passage"><span class="eyebrow">${esc(c.id)} / ${c.score} SHARED KEYWORDS</span><p>${esc(c.text)}</p><a href="${esc(c.url)}" target="_blank" rel="noopener noreferrer">${esc(c.title)} ↗</a><p class="small">Curated summary, not a verbatim quote.</p></article>`).join(''):'<p class="muted">No matching research passage. Try “privacy”, “booking” or “itinerary”.</p>';
}
...
const observer=new IntersectionObserver(entries=>{for(const e of entries){if(e.isIntersecting){e.target.classList.remove('pending');document.querySelectorAll('.pinned nav a').forEach(a=>a.classList.toggle('active',a.hash==='#'+e.target.id));}}},{threshold:.12});document.querySelectorAll('.chapter').forEach(s=>{if(!matchMedia('(prefers-reduced-motion: reduce)').matches)s.classList.add('pending');observer.observe(s);});
...
function renderRag(){
 ...
 if(!nodes.some(n=>n.id===ragSelection))ragSelection='context';
...
function renderFriction(){
 ...
 const counts=study.segments.map(s=>{const rows=study.respondents.filter(p=>p.segment===s.name);return {name:s.name,count:rows.filter(p=>threshold(p)>=price).length,n:rows.length};}).sort((a,b)=>b.count-a.count);
